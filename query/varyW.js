...
(async function () {
    ...
    let res = [[], [], [], []];
    // 测试不同哈希桶宽情况下的结果
    for (let width = 0.1; width <= 0.5; width += 0.02) {
        let stime = new Date().getTime();
        width = Number(width.toFixed(2));
        const { h1RandomLists, h2RandomLists, hashFamilys, hashTables, tableSize } =
            e2LSHIns.generateHashTable(dataSet, 10, 30, width, 10);
        ...
        for (let i = 0; i < indexs.length; i++) {
            ...
            // client:计算同一哈希桶内的点
            const searchRes =
                e2LSHIns.search(hashTables, hashFamilys, h1RandomLists, h2RandomLists, tableSize, q, width);
            if (!searchRes.length) {
                continue;
            }
            notNull += 1;
        ...
        console.log('notNull', notNull, '宽度', width, '精确度', accur / notNull, '候选集数量', candiNum / notNull, '运行时间', (etime - stime) / 1000 / notNull);
        res[0].push(width);
        res[1].push(accur / notNull);
        res[2].push(candiNum / notNull);
        res[3].push((etime - stime) / 1000 / notNull);
    }
    await writeJSON('./varyWRes.json', res);
